...
export default function CreateTaskForm({ onSubmit, loading = false, onCancel, initialData }: CreateTaskFormProps) {
  const [formData, setFormData] = useState<TaskFormData>({
    ...
    maxVolunteers: initialData?.maxVolunteers || 1,
  ...
  const validateForm = () => {
    ...
    // Time validation - if date is today, ensure time is not in the past
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Reset form on successful submission only if it's not editing mode
      if (!initialData) {
        setFormData({
          title: "",
          description: "",
    ...
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50">
        ...
        <p className="text-slate-600 mt-2">
          {initialData ? 'Update your volunteer opportunity details' : 'Create an engaging volunteer opportunity with proper date validation and required skills'}
        </p>
      </CardHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-3 pt-4 border-t">
            ...
            <Button
              type="submit"
              disabled={loading}
              className="flex-1 bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-700 hover:to-emerald-700 text-white shadow-lg"
            >
